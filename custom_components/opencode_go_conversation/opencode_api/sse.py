"""
SSE transport layer — low-level parsing of the OpenCodeGo streaming response.

Adapted from the upstream OpenCode Go streaming pipeline:
  raw byte-stream → SSE frames → typed ResponseEvent objects
"""
...
def _classify_error(code: str, message: str) -> OpenCodeGoError:
    """Map an API error code string to the correct exception subclass."""
    lc = code.lower()
    if "context_length_exceeded" in lc or "context_window" in lc:
        return OpenCodeGoContextWindowExceeded(message)
    if "quota" in lc or "insufficient_quota" in lc:
        return OpenCodeGoQuotaExceeded(message)
    if "usage_not_included" in lc:
        return OpenCodeGoUsageNotIncluded(message)
    if "rate_limit" in lc:
        return OpenCodeGoRateLimited(message)
    if "server_error" in lc or "overloaded" in lc:
        return OpenCodeGoServerOverloaded(message)
    return OpenCodeGoApiError(0, f"{code}: {message}")
...
def parse_event(data_str: str) -> ResponseEvent | None:
    ...
    can simply skip them; raises a ``OpenCodeGoError`` subclass for fatal or
    ...
    _LOGGER.debug("opencode_api.sse: unknown event type %r", etype)
    return None
...
async def sse_iter(resp: aiohttp.ClientResponse) -> AsyncIterator[ResponseEvent]:
    ...
    Follows the upstream OpenCode Go streaming pipeline.
